...
##
## stack_2_s3_bucket bucket
##

resource "aws_s3_bucket" "stack_2_s3_bucket" {
  bucket = "${var.project}-${var.tier}-${var.region}-stack-2"
  acl = "private"
  ...
  tags {
      Name = "${var.project}-${var.tier}-${var.region}-stack-2"
      Environment = "${var.tier}"
  }
}

output "stack_2_s3_bucket" {
    value = "${aws_s3_bucket.stack_2_s3_bucket.id}"
}
